...
void CAxisDlg::OnButton1() 
{
    ...
    if(m_check1==FALSE&&m_check2==FALSE)
         {MessageBox("Ⅰ导联还未进行选择！","错误",MB_OK|MB_ICONQUESTION);
          break;}
	if(m_check3==FALSE&&m_check4==FALSE)
         {MessageBox("Ⅱ导联还未进行选择！","错误",MB_OK|MB_ICONQUESTION);
          break;}
	if(m_check5==FALSE&&m_check6==FALSE)
         {MessageBox("Ⅲ导联还未进行选择！","错误",MB_OK|MB_ICONQUESTION);
          break;}
	if(m_check7==FALSE&&m_check8==FALSE)
         {MessageBox("aVL导联还未进行选择！","错误",MB_OK|MB_ICONQUESTION);
          break;}
	if(m_check9==FALSE&&m_check10==FALSE)
         {MessageBox("aVF导联还未进行选择！","错误",MB_OK|MB_ICONQUESTION);
          break;}
	if(m_check11==FALSE&&m_check12==FALSE)
         {MessageBox("aVR导联还未进行选择！","错误",MB_OK|MB_ICONQUESTION);
          break;}
	...
	if(lead1)
	 ...
	 m_qrsl=qrsl;
	 m_qrsu=qrsu;
     if(qrs<=90&&qrs>=0) m_diagnosis="正常QRS轴。";
     if(qrs<=120&&qrs>90) m_diagnosis="QRS轴轻度左偏。";
	 if(qrs<=150&&qrs>120) m_diagnosis="QRS轴显著左偏。";
	 if(qrs<=180&&qrs>150) m_diagnosis="QRS轴相当程度左偏。";
	 if(qrs<0&&qrs>=-30) m_diagnosis="QRS轴轻度右偏。";
	 if(qrs<-30&&qrs>=-60) m_diagnosis="QRS轴显著右偏。";
	 if(qrs<-60&&qrs>=-90) m_diagnosis="QRS轴相当程度右偏。";
	 if(qrs<-90&&qrs>=-180) m_diagnosis="QRS轴严重右偏。";
...
{
	MessageBox("作者：杨川  电子邮件：dowrun@21cn.com  QQ：2060350  ","联系作者",MB_OK|MB_ICONINFORMATION);		
